...
class LeetCodeProfile(BaseModel):
    ...
    ranking: int = Field(..., description="Ranking of the user on LeetCode")
    problem_diff_counts: dict[Literal["easy", "intermediate", "hard"], int] = Field(default_factory=dict,
                                                                                    description="Counts of problems solved by difficulty level")
    ...
    most_common_languages: list[str] = Field(default_factory=list,
                                             description="List of most common programming languages used by the user")
